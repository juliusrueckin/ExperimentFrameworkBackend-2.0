...
class Experiment():
    def __init__(self, config):
        ...
        self.parser = Parser(config["outputs"] if "outputs" in config else [])
        self.writer = CSVWriter(self.result_dir, self.name, self.command.env, self.command.cmd, self.parser.names())
        self.plotter = Plotter(config["plots"] if "plots" in config else [], self.result_dir, self.name, self.parser.names(), self.params.names)
